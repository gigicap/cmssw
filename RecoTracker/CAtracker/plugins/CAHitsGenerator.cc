...
theSeedingLayerToken(iC.consumes<SeedingLayerSetsHits>(cfg.getParameter<edm::InputTag>("SeedingLayers")))
	...
	//produce debug tree? NB: Use only with CAtest_cfg
	makehistos = false;
	if(m_debug > 1)
		makehistos = true;
...
//void CAHitsGenerator::hitSets(OrderedMultiHits& resultCA, const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   auto t_total = std::chrono::system_clock::now();
    ...
    edm::Handle<SeedingLayerSetsHits> hlayers;
     //std::cout<<"handle hlayers"<<std::endl;
    iEvent.getByToken(theSeedingLayerToken, hlayers);
     //std::cout<<"ievent getbytoken"<<std::endl;
    ...
    for(const auto& setAndLayers: trilayers) {
        ...
        std::cout<<"scoll size : "<<scoll->size()<<std::endl;
        trilaycount++;
        }
    	    std::cout<<"Out of the trilay : "<<trilaycount<<std::endl;
   ...
   theLayerCache.clear();
    ...
    for (size_t is = 0; is<scoll->size(); is++) {
        ...
        if (m_debug>0)   std::cout << "Starting hit usage : " << is << std::endl;
     
             
        if (m_debug>0)   
        		std::cout << "scoll[is] size (should be 3)"<<(*scoll)[is].size()<<std::endl;
        CAcellGenerator(builder, (*scoll)[is], is);
            if (m_debug > 0) 
            	std::cout << "Cells have been generated " << std::endl;
...
void CAHitsGenerator::CAcellGenerator(edm::ESHandle<TransientTrackingRecHitBuilder> & builder, SeedingHitSet sset,  int seednumber){
    //Build the CA cells, i.e. triplets (objects containing RecHits and fit parameters
        if (m_debug>0) std::cout << " building a ca cell "<< std::endl;
   ...
   if (m_debug>0) std::cout << " starting triplet size:  "<<starting_triplet.size()<< std::endl;
    ...
    if (m_debug>0) std::cout << " setting hi0/1/2 "<< std::endl;
...
/*    unsigned int sdidin =hi0->geographicalId().subdetId();
    ...
    std::cout<<"subdetids = "<<sdidout<<std::endl;*/
      ...
      if (m_debug>0) std::cout << " setting rawId's "<< std::endl;

      trip.rawId0 = OmniRef(hi0);
      trip.rawId1 = OmniRef(hi1);
      trip.rawId2 = OmniRef(hi2);

      if (m_debug>0) std::cout << " setting global points / 1"<< std::endl;
    ...
    GlobalPoint p2 = hi2->globalPosition();
          ...
          if (m_debug>0) std::cout << " setting global points / 2"<< std::endl;
    ...
    trip.dEta = DeltaEta(p0,p2);
          ...
          if (m_debug>0) std::cout << " setting detId's "<< std::endl;
