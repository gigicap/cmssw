...
# 1st seeding: triplets (to be modified)
...
process.CAtracker = cms.EDProducer('CAtracker',SeedColl=cms.untracked.InputTag('initialStepSeedsGigi'),EtaCut=cms.double(0.001))

#2nd seeding (final multiplets)
import RecoTracker.TkSeedGenerator.SeedGeneratorFromRegionHitsEDProducer_cfi
process.globalSeedsFromCAMultiplets = RecoTracker.TkSeedGenerator.SeedGeneratorFromRegionHitsEDProducer_cfi.seedGeneratorFromRegionHitsEDProducer.clone(
     OrderedHitsFactoryPSet = cms.PSet(
      ComponentName = cms.string('StandardMultiHitGenerator'),
      SeedingLayers = cms.string('initialStepSeedLayers2'),
      GeneratorPSet = cms.PSet(CAtracker)
    )
)
...
process.s = cms.Sequence(process.siPixelRecHits + process.siStripMatchedRecHits + process.initialStepSeedsGigi + process.CAtracker + process.globalSeedsFromCAMultiplets)
